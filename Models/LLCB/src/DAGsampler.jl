# To provide general sampling functinos for DAG structures 
# struct DAGSampler <: Sampleable{Matrixvariate, Continuous}
#     nv::Integer
# end

# 1. DAG and DAGSampler Structs:
struct DAG <: ContinuousMatrixDistribution
    nv::Integer # nv: the number of nodes
    logc0::AbstractFloat    # logc0: a constant used for log calculations, initialized to 0.0
end
...
# 2. DAGScorer Struct
struct DAGScorer    # DAGScorer represents the parameters for calculating a Beyasian Guassian Equivalent (BGE) score
    G::BitMatrix # unweighted adjacency matrix of the graph
    ESS::Integer    # Equivalent Sample Size
    # T0: a prior matrix for Wishart distribution based on node count and prior parameters au and aw
    T0::Matrix{AbstractFloat} # prior matrix for Wishart distribution
    aᵤ::AbstractFloat 
    aw::AbstractFloat
    nv::Integer # Number of nodes in the graph
end
...
# 3. bge Function (BGE Score Calcluation):
# Computes the Bayesian Gaussian Equialent (BGE) score of the DAG given data matrix X
# compute bge score
function bge(d::DAGScorer, X::AbstractMatrix{T}) where T<:AbstractFloat
    ...
    Σhat = cov(X)   # Sample covariance matrix
    # println("μhat = $μhat")
    # println("N = $N")
    ...
    # Updated covariance with prior, incorporating prior mean u0 and sample mean uhat
    ...
    # Iteratively calculates log-likelihoods (socres) for each node based on its parent nodes in G
    scores = zeros(N, d.nv)
...
# Returns the size of a DAG instance, (d.nv, d.nv), representing its adjacency matrix size.
Base.size(d::DAG) = (d.nv, d.nv)

# 4. Helper Functions:
function reverse(x::CartesianIndex) # Reverse the row and column indices in a CartesianIndex
    return CartesianIndex(last(Tuple(x)), first(Tuple(x)))
end

# 5. _rand! and rand Functions (DAG Sampling)
# _rand! is an internal function that modifies the adjacency matrix x to sample a new DAG
function _rand!(rng::AbstractRNG, d::DAG, x::Matrix{Float64}) 
    ...
    while count < max
        ...
        # Uses spectral radius (sr) to check if x is acyclic (small sr indicates no cycles)
...
# rand function uses _rand! to generate a valid adjacency matrix for a DAG
...
# 6. logkernel and _logpdf Functions:
# logkernel returns a zero score for a DAG object; logpdf uses an exponential function based on norm(X, 1) to return the log probability density for a given adjacency matrix X
...
function Distributions._logpdf(d::DAG, X::BitMatrix)
    ...
    γ = 3.5
    val = exp(-γ * norm(X, 1)) # Penalizes the number of edges in the DAG through norm(X, 1)
    return val
    # return 0.0
end
